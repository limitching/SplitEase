...
import { PageWrapper, ListWrapper, HeaderTextContainer, FixedButtonWrapper } from "../../components/PageWrapper";
...
const Debts = () => {
  ...
  const [selectSettlingDate, setSelectSettlingDate] = useState(dayjs(new Date()));
  ...
  const handleStartSettling = async selectSettlingDate => {
    ...
    const response = await api.startSettlingGroupDebts(group_id, startSettlingData, jwtToken);
  ...
  return (
    <>
      <PageWrapper>
        <HeaderTextContainer>
          <h5>
            {isNoDebts ? "No Debts :)" : group.minimized_debts === "1" ? "Total debts" : "Total debts (not minimized)"}
          </h5>
        </HeaderTextContainer>
        <ListWrapper>
        ...
        >
          <Tooltip title={"Only group owner can start settling"} placement="top">
            <div>
              <Button
                color="primary"
                size="large"
                ...
                sx={{
                  bgcolor: user.id === group.owner ? DASHBOARD_BG_COLOR : "transparent",
                  "&:hover": {
                    bgcolor: user.id === group.owner ? DASHBOARD_BG_COLOR : "transparent",
                    opacity: 0.87
                  },
      ...
      >
        <DialogTitle id="alert-dialog-title">{"Start Settling"}</DialogTitle>
        <DialogContent>
          <DialogContentText>
            <strong>Make sure that you want to start settling</strong>
            <br />
            Select a date to update all expenses with a date prior to the selected date from 'unsettled' to 'settling'
            status.
            <br />
          </DialogContentText>
          <br></br>
