...
import { List, ListItem, ListItemText, ListItemAvatar, Avatar, Divider, Tooltip } from "@mui/material";
    ...
    <>
      ...
      >
        ...
        {logs.map((log, index) => {
          ...
          const formattedDate = dayjs.utc(log.log_time).tz("Asia/Taipei").format("MM/DD/YYYY, h:mm:ss A");

          const eventTargetText = (event, event_target) => {
            if (event === "update all expenses with a date prior to") {
              return dayjs.utc(log.event.target).tz("Asia/Taipei").format("MM/DD/YYYY");
            }
            if (event === "create expense" || event === "update expense" || event === "delete expense") {
              if (event_target) {
                return event_target;
              } else {
                return "Expense";
            ...
            if (event === "create group" || event === "join group via code" || event === "archive group") {
              return "";
            }
              ...
              <ActivitiesContainer>
                <LogContainer>
                  <ListItem>
                    <ListItemAvatar>
                      <ListItemAvatar>
                        <Tooltip title={user?.name}>
                          <Avatar
                            alt={user?.name}
                            src={user?.image ? user.image : ANIMAL_AVATAR[indexMap.get(user?.id)]}
                            sx={{
                              width: 50,
                              height: 50
                            }}
                    ...
                    <Container>
                      <ListItemText
                        primary={`${user?.name} ${log?.event} ${eventTargetText(
                          log.event,
                          log.event_target
                        )} ${eventValueText(log?.event, log?.event_value)}`}
