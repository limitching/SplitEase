...
const ExpensesBlock = () => {
  const { members, memberMap, indexMap, groupExpense } = useContext(GroupContext);
  ...
  const handleExpenseItemClick = expense => {
    ...
    if (Object.keys(expense.creditors_amounts).length === 1) {
      const creditors = memberMap.get(Number(Object.keys(expense.creditors_amounts)[0]));
    ...
    const expenseChecked = Object.keys(expense.debtors_weight).map(debtorsId => memberMap.get(Number(debtorsId)));
    ...
    const localISOTime = new Date(gmtDate - timeZoneOffset).toISOString().substring(0, 16);
  ...
  return (
    <>
      <HeaderTextContainer>
        <h5>{groupExpense.length === 0 ? "No Expense :)" : "Expenses"}</h5>
      </HeaderTextContainer>
      ...
      >
        ...
        {groupExpense.map((expense, index) => {
          ...
          let creditors;
          if (Object.keys(expense.creditors_amounts).length === 1) {
            const creditorId = Object.keys(expense.creditors_amounts)[0];
            creditors = memberMap.get(Number(creditorId));
          } else {
            creditors = { name: "Multiple Members" };
          }
          const [currencyOption] = CURRENCY_OPTIONS.filter(currency => currency.id === expense.currency_option);

          const debtors = Object.keys(expense.debtors_weight).map(debtorId => {
            return memberMap.get(Number(debtorId));
          });

          return (
            <div key={expense._id}>
              <ListItem onClick={() => handleExpenseItemClick(expense)} id={index === 0 ? "expense-button" : null}>
                <ListItemButton>
                  <ListItemAvatar>
                    <Tooltip title={creditors?.name}>
                      <Avatar
                        alt={creditors?.name}
                        src={creditors?.image ? creditors.image : ANIMAL_AVATAR[indexMap.get(creditors?.id)]}
                  ...
                  <Container>
                    <ListItemText
                      primary={expense.description === "" ? "Expense" : `${expense.description}`}
                  ...
                  <Container style={{ maxWidth: "100px" }}>
                    <StyledListItemTextForAmount id={labelId} primary={`${currencyOption.symbol} ${expense.amount}`} />
                    <StyledAvatarGroup total={debtors.length} sx={{ fontSize: "12px" }}>
                      {debtors?.map((debtor, index) => (
                        <Tooltip key={expense._id + "_debtor_" + index} title={debtor?.name}>
                          <Avatar
                            alt={debtor?.name}
                            src={debtor?.image ? debtor.image : ANIMAL_AVATAR[indexMap.get(debtor?.id)]}
                            sx={{
                              width: "20px",
                              height: "20px",
                              fontSize: "12px"
                  ...
                  <Container style={{ maxWidth: "120px" }}>
                    <StyledListItemTextForAmount primary={<Chip label={expense.status} />} />
                  </Container>
                </ListItemButton>
              </ListItem>
              <Divider></Divider>
