...
import { CURRENCY_OPTIONS, ANIMAL_AVATAR } from "../../../../../global/constant";
import { useContext, useState } from "react";
import { TextField } from "@mui/material";
import { TbPlusMinus } from "react-icons/tb";
import { removeLeadingZeros, inputFormatter, amountFormatter } from "../../../../../utils/formatter";
import { List, ListItem, ListItemButton, ListItemText, ListItemAvatar, Checkbox, Avatar, Tooltip } from "@mui/material";
...
const DebtorsBlock = () => {
  ...
  const updateDividedAmounts = (member, index) => {
    if (selectedSplitMethod === 0) {
      ...
      return parseFloat(Number((amount / 100) * subValues[index]).toFixed(2));
    } else if (selectedSplitMethod === 3) {
      const totalShares = subValues.reduce((acc, cur) => acc + cur, 0);
      return parseFloat(Number((amount / totalShares) * subValues[index]).toFixed(2));
    } else if (selectedSplitMethod === 4) {
      const totalAdjustments = subValues.reduce((acc, cur) => acc + cur, 0);
      return parseFloat(Number((amount - totalAdjustments) / members.length + subValues[index]).toFixed(2));
    }
  };
  ...
  const handleExactAmountChange = (index, newValue) => {
    if (selectedCreditor === "multi") {
      ...
      const currentTotal = newSubValues.reduce((accumulator, currentValue, currentIndex) => {
      ...
      const toDistribute = remaining / (newSubValues.length - newModifiedIndices.length);
  ...
  return (
    <div className="debtor-list" style={{ width: "100%" }}>
      <Container as={Row} className="debtor-header-container" style={{ margin: 0, padding: 0 }}>
        {selectedCreditor === "multi" ? <Container style={{ height: "94.41px" }}></Container> : null}

        <Form.Label column lg="6" xs="6" style={{ display: "flex", alignItems: "center" }}>
          For whom
        </Form.Label>
        <Col
          lg="6"
        ...
        >
          ...
          >
            <Tooltip title="Introduction of split methods" placement="top">
            ...
            if (selectedSplitMethod === 0) {
              return (
                ...
                >
                  <ListItemButton>
                    <ListItemAvatar>
                      <Avatar alt={`${member.name}`} src={`${member.image ? member.image : ANIMAL_AVATAR[index]}`} />
                    </ListItemAvatar>
                    <ListItemText
                      id={labelId}
                      primary={`${member.name}`}
                      secondary={`${selectedCurrencyObj.symbol} ${updateDividedAmounts(member)}`}
                    />
                  </ListItemButton>
                </ListItem>
              );
              ...
              return (
                <ListItem alignItems="center" key={member.id} disablePadding>
                  <ListItemButton>
                    <ListItemAvatar>
                      <Avatar alt={`${member.name}`} src={`${member.image ? member.image : ANIMAL_AVATAR[index]}`} />
                    </ListItemAvatar>
                    {selectedSplitMethod === 1 ? (
                      ...
                      <ListItemText
                        id={labelId}
                        primary={`${member.name}`}
                        secondary={`${selectedCurrencyObj.symbol} ${updateDividedAmounts(member, index)}`}
                      />
                    )}
                    {selectedSplitMethod === 1 ? (
                      <>
                        <TextField
                          type="text"
                          variant="standard"
                          value={Number(subValues[index].toFixed(2))}
                          inputProps={{
                            style: {
                              textAlign: "right"
                            }
                          ...
                          onChange={event => {
                            ...
                            handleExactAmountChange(index, Number(event.target.value));
                          }}
                          sx={{ width: "30%" }}
                        />
                        <ListItemText
                      ...
                      <>
                        <TextField
                          ...
                          sx={{ width: "30%" }}
                        />
                        <ListItemText id={labelId} primary={`%`} style={{ maxWidth: "1rem" }} />
                      </>
                      ...
                      <>
                        <TextField
                          ...
                          onChange={event => {
                            handleShareChange(index, Number(event.target.value));
                          }}
                          sx={{ width: "30%" }}
                        />
                        <ListItemText id={labelId} primary={`share(s)`} style={{ maxWidth: "3rem" }} />
                      </>
                      ...
                      <>
                        ...
                        <TextField
                          ...
                          onChange={event => {
                            ...
                            handleAdjustmentChange(index, Number(event.target.value));
                          }}
                          sx={{ width: "30%" }}
                        />
                      </>
