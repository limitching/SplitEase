...
const ProfileModal = ({
  ...
  const { user, jwtToken, logout, setJwtToken, setUser } = useContext(AuthContext);
  ...
  const updateUser = async event => {
    ...
    if (data.errors || data.error) {
      return MySwal.fire({
        ...
        html: <div>{data.errors.map(error => <p>{error.msg}</p>) || data.error}</div>,
        icon: "error",
        timer: 1000,
  ...
  const handleCopyCode = () => {
    navigator.clipboard.writeText(`/bind ${user.line_binding_code}`).then(
      () => {
      ...
      >
        <Form onSubmit={updateUser}>
          <Modal.Header closeButton as={Row}>
            ...
            <LogoutButton variant="light" onClick={logout} style={{ marginRight: "1rem" }}>
              Logout
            </LogoutButton>
          </Modal.Header>
          <StyledModalBody>
            <Container>
              ...
              >
                ...
                >
                  <input hidden accept="image/*" type="file" onChange={handlingUpload} />
                  <PhotoCamera />
                </IconButton>
              </Container>
            </Container>
          ...
          <Modal.Footer>
            <Container className="d-grid">
              <SaveButton variant="warning" type="submit" disabled={error}>
