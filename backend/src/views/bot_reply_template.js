          ...
          contents: [
            ...
            },
            {
              ...
              contents: [
                {
                  type: "text",
                  text: group.description || defaultDescription,
                  wrap: true,
                  weight: "bold",
                  size: "xl",
                  flex: 0,
...
const generateGroupOverView = (
  ...
  const totalUserShare = groupExpenses.reduce(
    (acc, { debtors_weight, debtors_adjustment, amount }) => {
      ...
      const totalAdjustments = Array.from(debtors_adjustment.values()).reduce(
        (acc, curr) => acc + curr,
        0
      );
            ...
            },
            {
              ...
              contents:
                userDebts.length === 0
                  ...
                  : userDebts.map(([debtorIndex, creditorIndex, amount]) => {
                      return {
                        type: "box",
                        layout: "horizontal",
                        margin: "none",
...

            {
              ...
              contents:
                userCredits.length === 0
                  ? []
                  : userCredits.map(([debtorIndex, creditorIndex, amount]) => {
                      return {
                        type: "box",
                        layout: "horizontal",
                        margin: "none",
