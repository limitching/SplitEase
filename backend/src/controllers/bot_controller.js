...
import { getSettlingExpensesByGroupId } from "../models/expense_model.js";
import {
  createCurrencyGraph,
...
async function handleText(message, replyToken, source) {
  ...
  if (message.text.startsWith("/attach") || message.text.startsWith("/連接")) {
...
async function accessGroupPostBack(data, replyToken, source) {
  ...
  for (const [currency_option, graph] of Object.entries(currencyGraph)) {
    ...
    for (let i = 0; i < settlementTransactions[currency_option].length; i++) {
      const settledDebt = settlementTransactions[currency_option][i];
      for (let j = 0; j < currencyTransactions[currency_option].length; j++) {
